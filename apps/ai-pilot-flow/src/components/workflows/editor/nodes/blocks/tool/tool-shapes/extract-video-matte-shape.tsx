...
function ExtractVideoMatteGlyph({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 16 16"
      fill="none"
...
export function renderExtractVideoMatteTitle({
  title,
  tool,
}: ToolRendererProps) {
  return (
    <span className="inline-flex items-center gap-2">
      <ExtractVideoMatteGlyph className="size-4 text-white/82" />
      <span>{title || tool.label}</span>
    </span>
  )
}

export function renderExtractVideoMatteBody({ isRunning }: ToolRendererProps) {
  return (
